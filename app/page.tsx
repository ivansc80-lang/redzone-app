...
export default function Home() {
  ...
  useEffect(() => {
    setUsuarios(prevUsuarios => {
      ...
      return nuevosUsuarios.map((usr, index) => ({ ...usr, posicion: `${index + 1}º`, esLider: index === 0 }));
    });
  ...
  useEffect(() => {
    const cargarEstadoJornada = async () => {
      const { data, error } = await supabase.from('jornadas_eventos').select('estado').eq('jornada', jornadaActual).maybeSingle();
      if (error) { console.error('Error al cargar estado de jornada:', error); return; }
      if (data?.estado) setEstadoJornadaActual(data.estado);
    };
    cargarEstadoJornada();
  }, [jornadaActual]);
  ...
  const handleConfirmarPronosticos = async () => {
    ...
    if (error) { console.error('Error al guardar pronósticos en Supabase:', error); alert('Hubo un error al guardar tus pronósticos en Supabase.'); return; }
    setPronosticosPorUsuario(prev => {
      const jornadaData = prev[jornadaActual] || {};
      const usuarioActualData = jornadaData[usuarioActivoId] || { pronosticos: [], confirmado: false };
    ...
    <div className="min-h-screen bg-[#8b0000] text-white w-full font-sans">
      ...
      <main className="w-full pb-12">
        ...
        <div className="w-full bg-black/60 border-y border-red-900/60 py-2.5 px-4 flex flex-wrap justify-between items-center gap-3">
          <div className="flex items-center gap-3"><button onClick={() => setModoTest(!modoTest)} className={`px-3 py-1 rounded-lg text-xs font-black font-['Orbitron'] uppercase ${modoTest ? 'bg-red-600 text-white animate-pulse' : 'bg-zinc-800 text-zinc-400'}`}>{modoTest ? 'MODO TEST' : 'MODO NORMAL'}</button><span className="text-xs font-['Orbitron'] font-bold text-red-200">Jornada Activa: <strong className="text-white">{jornadaActual} / 18</strong></span></div>
          {modoTest && <div className="flex items-center gap-2 flex-wrap"><button onClick={handleVotacionAleatoriaYSimular} className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold py-1.5 px-3 rounded-lg font-['Orbitron'] uppercase shadow transition-all cursor-pointer">VOTACIÓN ALEATORIA (JUANJO & CACE) + SIMULAR</button><button onClick={handleSiguienteJornada} className="bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold py-1.5 px-3 rounded-lg font-['Orbitron'] uppercase shadow transition-all cursor-pointer flex items-center gap-1">SIGUIENTE JORNADA →</button></div>}
        </div>
        ...
        <div className="p-4 md:p-8 w-full max-w-[1600px] mx-auto">
            ...
            <section className="space-y-8 bg-[#8b0000] p-2 md:p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">{usuarios.map((usr) => {
                ...
                return <div key={usr.id} className="flex flex-col space-y-3 bg-black/40 p-3 rounded-2xl border border-red-900/60 shadow-xl"><div className="w-full aspect-video bg-black rounded-xl overflow-hidden border-2 border-red-900/80 shadow-2xl relative group"><img src={usr.avatarJornada} alt={usr.nombre} className="w-full h-full object-cover" /></div><div className="space-y-1"><div className={`${usr.colorBg} border border-white/20 rounded-t-lg py-3 text-center text-white font-['Orbitron'] font-black text-2xl leading-none shadow`}>{confirmadoUsr ? `${puntosJornadaActual} aciertos` : '0 aciertos'}</div><div className={`${usr.colorBg} border border-white/20 rounded-b-lg py-3 text-center text-white font-['Orbitron'] font-bold text-xl leading-none tracking-wider`}>Total Acumulado: {usr.puntos} pts</div></div><div className="bg-black/90 border border-zinc-800 rounded-xl overflow-hidden shadow-xl p-2 space-y-2">{pronosticosUsr.map((p) => {
                  ...
                  return <div key={p.id} className="grid grid-cols-[1fr_auto_1fr_auto] items-center gap-1.5 bg-[#2a2a2a] px-2.5 py-2 rounded transition-colors border border-zinc-700/50"><div className="flex items-center justify-center gap-1.5 min-w-0"><img src={p.localLogo} alt={p.local} className={`object-contain flex-shrink-0 ${p.local === 'Jets' ? 'w-8 h-8 scale-125 filter brightness-200' : 'w-5 h-5 md:w-6 md:h-6'}`} /><span className="font-['Orbitron'] font-bold text-white truncate text-[0.7rem] md:text-[0.8rem] text-center uppercase">{p.local}</span></div><span className="font-['Orbitron'] font-bold text-zinc-300 text-[0.65rem] md:text-[0.75rem] px-0.5 text-center">vs</span><div className="flex items-center justify-center gap-1.5 min-w-0"><span className="font-['Orbitron'] font-bold text-white truncate text-[0.7rem] md:text-[0.8rem] text-center uppercase">{p.visitante}</span><img src={p.visitanteLogo} alt={p.visitante} className={`object-contain flex-shrink-0 ${p.visitante === 'Jets' ? 'w-8 h-8 scale-125 filter brightness-200' : 'w-5 h-5 md:w-6 md:h-6'}`} /></div><div className={`w-6 h-6 md:w-7 md:h-7 flex items-center justify-center border rounded font-['Orbitron'] font-black text-xs md:text-sm ml-1 flex-shrink-0 justify-self-end ${estiloCajaEleccion}`}>{eleccion === '1' ? '1' : eleccion === 'X' ? 'X' : eleccion === '2' ? '2' : '-'}</div></div>;
                })}</div></div>;
              })}</div>
            </section>
          )}
          ...
          {pestanaActiva === 'equipos' && (
            <section className="space-y-6"><div className="flex justify-center items-center gap-4 border-b border-red-900/50 pb-4"><button onClick={() => setSubPestanaEquipos('score')} className={`px-8 py-2 font-['Orbitron'] text-xs md:text-sm font-bold uppercase rounded-lg transition-all border border-white/20 cursor-pointer ${subPestanaEquipos === 'score' ? 'bg-white text-black shadow-lg scale-105' : 'bg-[#5c0000] text-white hover:bg-[#7a0000]'}`}>SCORE</button><button onClick={() => setSubPestanaEquipos('games')} className={`px-8 py-2 font-['Orbitron'] text-xs md:text-sm font-bold uppercase rounded-lg transition-all border border-white/20 cursor-pointer ${subPestanaEquipos === 'games' ? 'bg-white text-black shadow-lg scale-105' : 'bg-[#5c0000] text-white hover:bg-[#7a0000]'}`}>GAMES (18 JORNADAS)</button></div>{subPestanaEquipos === 'score' ? <div className="space-y-8">{sincronizandoPosiciones && <div className="flex justify-end"><span className="text-[10px] text-red-200 font-mono animate-pulse">Actualizando datos desde ESPN...</span></div>}<div className="space-y-4"><div className="border border-white bg-white p-4 rounded-xl space-y-4 shadow-xl"><div className="flex items-center gap-3 border-b-2 border-red-600 pb-2"><div className="w-3 h-3 bg-red-600 rounded-full animate-pulse" /><h3 className="text-base md:text-xl font-black uppercase tracking-wider text-red-600 font-['Orbitron'] italic underline decoration-red-600 underline-offset-4">Conferencia Americana (AFC)</h3></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{divisiones.filter((d) => d.conferencia === 'AFC' || d.nombre.toUpperCase().includes('AFC')).map((div, idx) => renderTablaDivision(div, idx))}</div></div></div><div className="space-y-4 pt-4"><div className="border border-white bg-white p-4 rounded-xl space-y-4 shadow-xl"><div className="flex items-center gap-3 border-b-2 border-blue-500 pb-2"><div className="w-3 h-3 bg-blue-500 rounded-full animate-pulse" /><h3 className="text-base md:text-xl font-black uppercase tracking-wider text-blue-600 font-['Orbitron'] italic underline decoration-blue-600 underline-offset-4">Conferencia Nacional (NFC)</h3></div><div className="grid grid-cols-1 md:grid-cols-2 gap-6">{divisiones.filter((d) => d.conferencia === 'NFC' || d.nombre.toUpperCase().includes('NFC')).map((div, idx) => renderTablaDivision(div, idx))}</div></div></div></div> : <div className="space-y-8">{Array.from({ length: 18 }, (_, i) => i + 1).map((jNum) => { const jornadaValidada = pronosticosPorUsuario[jNum]?.['cace']?.validado; const partidosJornada = jornadasOficiales[jNum] || []; return <div key={jNum} className="bg-black/80 border border-red-900/80 rounded-2xl p-4 md:p-6 space-y-4 shadow-2xl"><div className="flex justify-between items-center border-b border-red-900/60 pb-3"><h3 className="text-base md:text-xl font-['Orbitron'] font-black text-amber-400 uppercase tracking-wider">JORNADA {jNum} DE 18 {jornadaValidada ? '(FINALIZADA / VALIDADA)' : '(PENDIENTE DE JUGAR)'}</h3><span className="text-xs font-mono text-zinc-400">{jornadaValidada ? 'Datos subidos y registrados' : 'Solo información básica de emparejamientos'}</span></div><div className="grid grid-cols-1 md:grid-cols-3 gap-4">{partidosJornada.map((partido: any) => { const local = partido.local || ''; const visitante = partido.visitante || ''; const localLogo = partido.localLogo || ''; const visitanteLogo = partido.visitanteLogo || ''; const rachaLocalAcumulada = obtenerRachaEquipo(local, jNum); const rachaVisitanteAcumulada = obtenerRachaEquipo(visitante, jNum); return <div key={partido.id} className="bg-[#1a1a1a] border border-zinc-800 rounded-xl p-3 flex flex-col gap-3 shadow-md"><div className="flex justify-between items-center bg-black/40 p-2 rounded-lg border border-zinc-800"><div className="flex items-center gap-2"><img src={localLogo} alt={local} className="w-7 h-7 object-contain" /><span className="font-['Orbitron'] font-bold text-xs text-white uppercase">{local}</span></div><span className="font-['Orbitron'] font-bold text-xs text-zinc-500">vs</span><div className="flex items-center gap-2"><span className="font-['Orbitron'] font-bold text-xs text-white uppercase">{visitante}</span><img src={visitanteLogo} alt={visitante} className="w-7 h-7 object-contain" /></div></div><div className="flex justify-between items-center text-[10px] font-mono text-zinc-400 px-1 border-b border-zinc-800 pb-2"><span>RACHA LOCAL: <strong className="text-emerald-400">{rachaLocalAcumulada}</strong></span><span>RACHA VIS.: <strong className="text-red-400">{rachaVisitanteAcumulada}</strong></span></div><div className="space-y-1"><span className="text-[10px] font-mono uppercase text-zinc-400 tracking-wider">Pronósticos de Participantes:</span><div className="grid grid-cols-3 gap-1.5 text-center">{usuarios.map(usr => { const eleccionUsr = pronosticosPorUsuario[jNum]?.[usr.id]?.pronosticos?.find(p => p.local === local || p.id === partido.id)?.eleccion || '-'; return <div key={usr.id} className="bg-black/60 border border-zinc-800 rounded p-1 flex flex-col"><span className="text-[9px] font-['Orbitron'] font-bold text-zinc-400 uppercase">{usr.nombre}</span><span className="text-xs font-mono font-black text-amber-400 mt-0.5">{eleccionUsr}</span></div>; })}</div></div></div>; })}</div></div>; })}</div>}</section>
          )}

          {pestanaActiva === 'noticias' && <section className="space-y-4"><h2 className="text-xs md:text-sm font-black uppercase tracking-wider text-red-200 border-b border-red-900/50 pb-1 font-['Orbitron'] italic">Última Hora NFL (Español)</h2>{cargandoNoticias ? <div className="p-8 text-center text-red-200 font-['Orbitron'] animate-pulse">Cargando noticias en castellano...</div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{noticias.map((item) => <article key={item.id} className="bg-black/90 border border-red-900/60 rounded-xl overflow-hidden flex flex-col justify-between hover:border-red-600 transition-all shadow-lg"><div>{item.imagen && <div className="h-44 w-full overflow-hidden bg-zinc-900"><img src={item.imagen} alt={item.titulo} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" /></div>}<div className="p-4 space-y-2"><span className="text-[10px] font-mono text-red-400 font-semibold uppercase">{item.fecha}</span><h3 className="font-['Orbitron'] text-sm font-bold leading-snug text-white">{item.titulo}</h3><p className="text-xs text-zinc-300 line-clamp-3 leading-relaxed">{item.descripcion}</p></div></div><div className="p-4 pt-0"><a href={item.enlace} target="_blank" rel="noopener noreferrer" className="inline-block w-full text-center bg-red-950/80 hover:bg-red-900 text-red-100 text-xs font-['Orbitron'] py-2 rounded border border-red-800 transition-colors uppercase">Leer Noticia Completa 🇪🇸</a></div></article>)}</div>}</section>}

          {pestanaActiva === 'perfil' && <section className="space-y-4 max-w-md mx-auto"><h2 className="text-sm md:text-base font-black uppercase tracking-wider text-red-200 border-b border-red-900/50 pb-2 font-['Orbitron']">Ajustes de Perfil Privado</h2>{!usuarioLogueado ? <div className="bg-black/90 border border-red-800 rounded-xl p-6 space-y-4 shadow-xl"><h3 className="text-sm font-['Orbitron'] text-white font-bold uppercase text-center">Iniciar Sesión</h3>{errorLogin && <p className="text-red-400 text-xs text-center font-mono">{errorLogin}</p>}<input type="email" placeholder="Correo electrónico" value={emailInput} onChange={(e) => setEmailInput(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded-lg p-2.5 text-white text-xs font-mono focus:outline-none" /><div className="relative"><input type={verPassword ? 'text' : 'password'} placeholder="Contraseña" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} className="w-full bg-zinc-900 border border-zinc-700 rounded-lg p-2.5 pr-10 text-white text-xs font-mono focus:outline-none" /><button type="button" onClick={() => setVerPassword(!verPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-400 hover:text-white text-sm focus:outline-none cursor-pointer">{verPassword ? '👁️‍🗨️' : '👁️'}</button></div><button onClick={handleLogin} className="w-full py-2.5 bg-red-700 hover:bg-red-800 text-[#fff] font-['Orbitron'] text-xs font-bold rounded-lg uppercase transition-colors shadow-lg cursor-pointer">Entrar</button></div> : <div className="bg-white border border-red-800 rounded-xl p-6 space-y-4 shadow-2xl text-black font-['Orbitron']"><div className="text-center p-3 bg-[#9e0101] rounded-lg shadow-inner"><p className="text-[10px] text-red-200 uppercase">Sesión activa:</p><strong className="text-white text-xs font-mono">{usuarioLogueado.email}</strong></div><div className="space-y-3 text-xs"><div><label className="block text-[#002244] font-bold mb-1">Nombre de Usuario</label><input type="text" value={nombrePerfil} onChange={(e) => setNombrePerfil(e.target.value)} placeholder="Ej: Cace" className="w-full bg-[#9e0101] text-[#ffffff] placeholder-red-200 border border-red-700 rounded-lg p-2.5 font-mono focus:outline-none" /></div><div><label className="block text-[#002244] font-bold mb-1">Nombre de tu Equipo</label><input type="text" value={nombreEquipo} onChange={(e) => setNombreEquipo(e.target.value)} placeholder="Ej: PATRIOTS" className="w-full bg-[#9e0101] text-[#ffffff] placeholder-red-200 border border-red-700 rounded-lg p-2.5 font-mono focus:outline-none" /></div><div><label className="block text-[#002244] font-bold mb-1">URL del Avatar</label><input type="text" value={avatarUrlInput} onChange={(e) => setAvatarUrlInput(e.target.value)} placeholder="https://..." className="w-full bg-[#9e0101] text-[#ffffff] placeholder-red-200 border border-red-700 rounded-lg p-2.5 font-mono focus:outline-none" /></div></div><button onClick={handleGuardarPerfil} disabled={guardandoPerfil} className="w-full py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-lg uppercase transition-colors shadow-lg cursor-pointer">{guardandoPerfil ? 'Guardando...' : 'Guardar Cambios'}</button><button onClick={handleLogout} className="w-full py-2 bg-[#9e0101] hover:bg-red-900 text-white text-xs font-bold rounded-lg border border-red-700 transition-colors uppercase cursor-pointer shadow">Cerrar Sesión</button></div>}</section>}
        </div>
      </main>
    </div>
  );
