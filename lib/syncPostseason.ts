...
async function obtenerUltimaJornadaRegular(temporada: number) {
  ...
  if (configError) {
    throw new Error(`Error leyendo control horario de playoffs: ${configError.message}`);
  }
  ...
  if (!tocaComprobarEspn && desdeBbdd) {
    return desdeBbdd;
  }
...
async function cerrarPorraPlayoffSiProcede(params: {
  temporada: number;
  jornada: number;
  estado: string | null;
  ...
  if (
    estado !== 'pendiente' ||
    !cierrePronosticos ||
    Date.now() < new Date(cierrePronosticos).getTime()
  ) {
    return estado;
  }
  ...
  if (error) {
    throw new Error(`Error cerrando PORRA J${jornada} de playoffs: ${error.message}`);
  }
    ...
    try {
      ...
      for (const evento of eventos) {
        ...
        if (!comp || !id || !localAbrev || !visitAbrev || Number.isNaN(fecha.getTime())) {
          throw new Error(`${definicion.nombre} contiene un partido sin id, equipos o fecha/hora válidos.`);
        }
        if (localAbrev === visitAbrev) {
          throw new Error(`${definicion.nombre}: ESPN devuelve el mismo equipo como local y visitante en ${id}.`);
        }
      ...
      for (const evento of eventos) {
        ...
        const { data: partidoGuardado, error: upsertError } = await supabase
          ...
          .upsert({
            ...
            fecha_partido: fechaPartido, estado, puntos_local: puntosLocal,
            puntos_visitante: puntosVisitante, periodo, reloj,
        ...
        if (upsertError) {
          throw new Error(`Error al guardar partido ${definicion.nombre} ESPN ${evento.id}: ${upsertError.message}`);
        }
      ...
      if (verificados?.length !== definicion.partidosEsperados || eventos.some((e: any) => !idsVerificados.has(String(e.id)))) {
      ...
      if (jornadaVerificadaError || !jornadaVerificada) {
        throw new Error(`No se pudo verificar la jornada administrativa ${definicion.nombre}: ${jornadaVerificadaError?.message || 'sin registro'}`);
      }
      if (!jornadaVerificada.inicio_jornada || !jornadaVerificada.cierre_pronosticos) {
        throw new Error(`${definicion.nombre} no quedó con inicio_jornada/cierre_pronosticos válidos.`);
      }

      const estadoFinal = await cerrarPorraPlayoffSiProcede({
        temporada,
        jornada,
      ...
      resultados.push({
        ronda, nombre: definicion.nombre, preparada: true, jornada,
        semanaEspn: localizacion.week, partidos: eventos.length,
        estado: estadoFinal,
