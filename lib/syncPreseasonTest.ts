...
const TEST_FIRST_GAME = new Date('2026-08-21T08:00:00.000Z'); // 10:00 España
const TEST_PICK_CLOSE = new Date('2026-08-21T07:30:00.000Z'); // 09:30 España
...
export async function sincronizarPretemporadaTest(): Promise<PreseasonSyncResult> {
  ...
  if (ahora >= limite) {
    const { error: desactivarError } = await supabase
      ...
      .eq('id', 1);
    if (desactivarError) throw new Error(`Error al desactivar el modo de pretemporada: ${desactivarError.message}`);

    const { error: restaurarJornadaError } = await supabase
      .from('jornadas_eventos')
      .update({ estado: 'pendiente' })
      .eq('jornada', 1);
    if (restaurarJornadaError) throw new Error(`Error al restaurar la jornada 1: ${restaurarJornadaError.message}`);
  ...
  if (eventos.length === 0) throw new Error('ESPN no devolvió partidos de pretemporada para la semana 3 de 2026.');
  ...
  for (const evento of eventos) {
    ...
    if (completado) resultadoOficial = puntosLocal > puntosVisitante ? '1' : puntosLocal < puntosVisitante ? '2' : 'X';
    ...
    const { data: partidoGuardado, error: upsertError } = await supabase
      ...
      .upsert({
        ...
        fecha_partido: localAbrev === 'HOU' && visitAbrev === 'LV' ? TEST_FIRST_GAME.toISOString() : new Date(evento.date).toISOString(),
        estado,
    ...
    if (upsertError) throw new Error(`Error al guardar partido de pretemporada ESPN ${evento.id}: ${upsertError.message}`);

    if (completado && resultadoOficial && partidoGuardado) {
      const { error: aciertosError } = await supabase.from('pronosticos').update({ acierto: true }).eq('partido_id', partidoGuardado.id).eq('eleccion', resultadoOficial);
      if (aciertosError) throw new Error(`Error al validar aciertos de pretemporada: ${aciertosError.message}`);
      const { error: fallosError } = await supabase.from('pronosticos').update({ acierto: false }).eq('partido_id', partidoGuardado.id).neq('eleccion', resultadoOficial);
      if (fallosError) throw new Error(`Error al validar fallos de pretemporada: ${fallosError.message}`);
    }
  }

  const estadoPretemporada: 'pendiente' | 'cerrada' | 'finalizada' = todosFinalizados ? 'finalizada' : ahora >= cierrePronosticos ? 'cerrada' : 'pendiente';
  ...
  const { error: configUpdateError } = await supabase
    ...
    .eq('id', 1);
  if (configUpdateError) throw new Error(`Error al actualizar app_config de pretemporada: ${configUpdateError.message}`);

  const { error: jornadaError } = await supabase
    .from('jornadas_eventos')
    .update({ inicio_jornada: primerPartidoFecha.toISOString(), cierre_pronosticos: cierrePronosticos.toISOString(), fin_jornada: ultimoPartidoFecha.toISOString(), estado: estadoPretemporada })
    .eq('jornada', 1);
  if (jornadaError) throw new Error(`Error al preparar jornada 1 para la prueba de pretemporada: ${jornadaError.message}`);
