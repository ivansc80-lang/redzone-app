...
const configurarWebPush = () => {
  ...
  if (!publicKey || !privateKey) {
    throw new Error(
      "Faltan claves VAPID para enviar PUSH.",
    );
  }
...
export const eventoPushYaEnviado = async (
  claveEvento: string,
) => {
  ...
  if (error) {
    throw new Error(
      `Error comprobando PUSH enviado: ${error.message}`,
    );
  }
...
export const enviarPushRedzone = async ({
  ...
  if (suscripcionesError) {
    throw new Error(
      `Error leyendo suscripciones PUSH: ${suscripcionesError.message}`,
    );
  }
  ...
  if (enviados > 0) {
    ...
    if (logError) {
      throw new Error(
        `PUSH enviado pero no se pudo registrar: ${logError.message}`,
      );
    }
  }
...
export const guardarLogro = async ({
  ...
  const { error } = await supabase
    ...
    .upsert(
      ...
      {
        onConflict:
          "user_id,temporada,jornada,tipo_competicion,tipo_logro",
      },
    );

  if (error) {
    throw new Error(
      `Error guardando logro: ${error.message}`,
    );
  }
};
