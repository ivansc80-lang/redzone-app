...
export async function fetchAndSyncJornadaData(jornadaNum) {
  try {
    console.log("➡️ Solicitando partidos para la jornada:", jornadaNum);
    ...
    console.log("📥 Datos recibidos de Supabase en 'partidos':", partidosDB);

    if (error) throw error;
    if (!partidosDB || partidosDB.length === 0) {
      console.warn("⚠️ No hay partidos en la base de datos para esta jornada.");
      return [];
    }
