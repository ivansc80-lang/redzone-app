...
function esTemporadaSinEstadisticas(error: unknown) {
  const mensaje =
  ...
  return (
    mensaje.includes("ESPN API error 404") ||
    mensaje.includes("ESPN 404") ||
    mensaje.includes("404: Not Found")
  );
}

async function devolverVacioSiNoHayTemporada<T>(
  cargar: () => Promise<T[]>,
): Promise<T[]> {
  try {
    return await cargar();
  } catch (error) {
    if (esTemporadaSinEstadisticas(error)) {
      return [];
    }
...
export async function getTeamOffenseSummary(temporada=2025, seasonType=2): Promise<EspnTeamOffenseSummary[]> {
  const equipos = await devolverVacioSiNoHayTemporada(() =>
    getTeamOffenseLeaders("yardas_totales", temporada, seasonType),
  );
...
export async function getTeamDefenseSummary(temporada=2025, seasonType=2): Promise<EspnTeamDefenseSummary[]> {
  const equipos = await devolverVacioSiNoHayTemporada(() =>
    getTeamDefenseLeaders("yardas_permitidas", temporada, seasonType),
  );
  return equipos.map(e=>({posicion:e.posicion,teamId:e.teamId,nombre:e.nombre,equipo:e.equipo,GP:e.GP,
...
export async function getTeamSpecialTeamsSummary(temporada=2025, seasonType=2): Promise<EspnTeamSpecialTeamsSummary[]> {
  const equipos = await devolverVacioSiNoHayTemporada(() =>
    getTeamSpecialTeamsLeaders("devoluciones", temporada, seasonType),
  );
  return equipos.map(e=>({posicion:e.posicion,teamId:e.teamId,nombre:e.nombre,equipo:e.equipo,GP:e.GP,
...
export async function getTeamTurnoversSummary(temporada=2025, seasonType=2): Promise<EspnTeamTurnoversSummary[]> {
  return devolverVacioSiNoHayTemporada(() =>
    getTeamTurnoversLeaders("diferencial", temporada, seasonType),
  );
}
