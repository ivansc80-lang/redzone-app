...
export default function TeamTurnoversSummary() {
  ...
  useEffect(()=>{ let cancelado=false;
    ...
    } catch(err) { console.error("Error cargando Resumen entregas:",err); if(!cancelado) setError("No se pudo cargar el resumen desde ESPN."); }
  ...
  return <div className="w-full">
    <div className="flex items-center gap-3 border-b-2 border-red-600 pb-3 mb-4">
      ...
      <h3 className="text-sm md:text-xl font-black uppercase tracking-wider text-red-600 font-['Orbitron'] italic underline decoration-red-600 underline-offset-4">Resumen entregas</h3>
    </div>
    ...
    <div className="w-full overflow-x-auto border border-zinc-200 rounded-xl shadow-sm">
      <table className="min-w-[900px] w-full border-collapse text-xs">
        ...
        :datos.map(e=><tr key={e.teamId} className="border-b border-zinc-100 hover:bg-zinc-50">
          ...
          {keys.map((key,idx)=><td key={String(key)} className={`min-w-[78px] px-3 py-3 text-center whitespace-nowrap border-r border-zinc-100 ${idx===1?"font-black text-red-700":"text-zinc-600"}`}>{String(e[key]||"-")}</td>)}
        </tr>)}</tbody>
      </table>
    </div>
  </div>;
