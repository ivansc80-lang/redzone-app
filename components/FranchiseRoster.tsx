...
function salaryValue(a: Athlete) {
  ...
  const contract =
    a.contract?.baseSalary ?? a.contract?.salary ?? a.contract?.totalSalary;
  ...
  const value =
    direct ??
    contract ??
    (contracts.length ? Math.max(...contracts) : undefined);
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .catch((e) => {
        if (alive)
          setError(
            e instanceof Error ? e.message : "No se pudo cargar la plantilla",
          );
      })
      .finally(() => {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <h3 className="font-['Orbitron'] text-3xl font-black uppercase text-red-700 md:text-4xl">
          Plantilla
        </h3>
        {coachName && (
          <div className="font-['Orbitron'] text-xs font-black uppercase text-red-700 md:text-sm">
                    ...
                    <td className="px-4 py-2">
                      ...
                      >
                        {a.headshot?.href ? (
                          ...
                          <span className="h-10 w-10 rounded-full bg-zinc-100" />
                        )}
                        <span>
                          {name}
                          {a.jersey ? (
